...
parent = defaultdict[tuple[int, int, int], set[tuple[int, int, int]]](set)
...
while queue:
    ...
    for dir, (di, dj) in enumerate(dirs):
        ...
        if 0 <= new_i < HEIGHT and 0 <= new_j < WIDTH and grid[new_i][new_j] != "#":
            ...
            elif existing_distance == current_distance + cost:
                parent[next_pos].add(current)
...
path: dict[tuple[int, int], str] = {shortest_end[:2]: "E"}
p = parent[shortest_end]
dir_chars = ">v<^"
while p:
    pi = p.pop()
    path[pi[:2]] = dir_chars[pi[2]]
    p |= parent[pi]
